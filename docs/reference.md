...
*   [Buildfile functions](#functions)
    ...
    *   [mark_used: Marks variables as used from the current scope.](#func_mark_used)
    *   [mark_used_from: Marks variables as used from a different scope.](#func_mark_used_from)
...
*   [Built-in predefined variables](#predefined_variables)
    *   [color_console: [boolean] Whether or not colored output is on.](#var_color_console)
    ...
    *   [gn_version: [scope] GN version information.](#var_gn_version)
...
*   [Variables you set in targets](#target_variables)
    ...
    *   [asmppflags: [string list] Flags passed to the assembler pre-processor.](#var_asmppflags)
    ...
    *   [command: [string with substitutions] Command to run for actions.](#var_command)
    ...
    *   [console_pool: [label] Console pool object.](#var_console_pool)
    *   [contents: Contents to write to file.](#var_contents)
    *   [cppflags: [string list] Flags passed to all C pre-processor variants.](#var_cppflags)
    *   [cppflags_c: [string list] Flags passed to the C pre-processor.](#var_cppflags_c)
    *   [cppflags_cc: [string list] Flags passed to the C++ pre-processor.](#var_cppflags_cc)
    *   [cppflags_objc: [string list] Flags passed to the Objective C pre-processor.](#var_cppflags_objc)
    *   [cppflags_objcc: [string list] Flags passed to the Objective C++ pre-processor.](#var_cppflags_objcc)
    ...
    *   [description: [string with substitutions, optional] Description of the action.](#var_description)
    ...
    *   [interpreter: [string, optional] Interpreter to use for action scripts.](#var_interpreter)
    ...
    *   [sys_include_dirs: [directory list] Additional system include directories.](#var_sys_include_dirs)
...
### <a name="cmd_analyze"></a>**gn analyze**: Analyze which targets are affected by a list of files.
...
```
### <a name="cmd_args"></a>**gn args**: Display or configure arguments declared by the build.
...
```
  ...
  gn args --list --args="os=\"android\" enable_doom_melon=true"
    ...
    arguments).
```
### <a name="cmd_check"></a>**gn check**: Check header dependencies.
```
...
```
### <a name="cmd_clean"></a>**gn clean**: Cleans the output directory.
...
```
### <a name="cmd_desc"></a>**gn desc**: Show lots of insightful information about a target or config.
...
```
### <a name="cmd_format"></a>**gn format**: Format .gn file.
...
```
  ...
  gn format --stdin
```
### <a name="cmd_gen"></a>**gn gen**: Generate ninja files.
...
```
### <a name="cmd_ls"></a>**gn ls**: List matching targets.
```
...
```
### <a name="cmd_path"></a>**gn path**: Find paths between two targets.
...
```
### <a name="cmd_refs"></a>**gn refs**: Find stuff referencing a target or file.
...
```
  This target type allows you to run a script or command a single time to
  produce one or more output files. If you want to run a script once for each
...
```
  ...
  paths since GN doesn't know which of the args are paths and which are not.
  You will want to use rebase_path() to convert paths to be relative to the
  root_build_dir.
...
```
  This target type allows you to run a script or command once-per-file over a
  set of sources. If you want to run a script once that takes many files as
...
```
  Flags: arflags, asmflags, cflags, cflags_c, cflags_cc, cflags_objc,,
         ...
         cppflags_objcc, defines, include_dirs, inputs, ldflags,
...
```
  ...
  need to remove the corresponding config that sets it. The final order of
  flags, defines, etc. for a target is generated based on the config value in
  question.
  ...
  On the command-line, the precedence of include dirs and lib dirs is the same
  as the order specified on the command-line. However, for compile flags, the
  last flag on the command-line takes precedence over all preceding flags.
  Hence, the need for two separate orderings, one for dirs and one for flags
  and defines, so that values specified directly on the target take precedence
...
```
  ...
  in the current (destination) scope already, and clobber is flase, an error
  will be thrown, otherwise the current value is clobbered. If "*" is specified,
  variables in the current scope will be clobbered, regardless of the value of
  the clobber flag (the latter is important because most targets have an
...
```
  ...
  import("my_vars.gni")
```
### <a name="func_mark_used"></a>**mark_used**: Marks variables as used from the current scope.
...
```
### <a name="func_mark_used_from"></a>**mark_used_from**: Marks variables as used from a different scope.
...
```
  ...
  available:
    ...
    {{sys_include_dirs}}
        ...
        ones), and system include directories will be prefixed by "-isystem",
        unless these prefixes are overridden using the "define_switch",
        "include_switch", or "sys_include_switch" toolchain variables,
        respectively.
...
### <a name="var_color_console"></a>**color_console**: [boolean] Whether or not colored output is on.
...
```
### <a name="var_gn_version"></a>**gn_version**: [scope] GN version information.
...
```
  ...
  6. Those set on the current target (not in a config).
```
### <a name="var_asmppflags"></a>**asmppflags**: Flags passed to the assembler pre-processor.

```
  A list of strings.

  "asmppflags" are passed to any invocation of a tool that takes an .asm or .S
  file as input.
```
...
```
  ...
  See also "gn help create_bundle".
```
### <a name="var_command"></a>**command**: Command to run for actions.
...
```
### <a name="var_console_pool"></a>**console_pool**: Console pool object.
...
```
### <a name="var_cppflags"></a>**cppflags***: Flags passed to the pre-processor.

```
  A list of strings.

  "cppflags" are passed to all invocations of the C, C++, Objective C, and
  Objective C++ compilers.

  To target one of these variants individually, use "cppflags_c",
  "cppflags_cc", "cppflags_objc", and "cppflags_objcc", respectively. These
  variant-specific versions of cppflags* will be appended to the "cppflags".
...
```
  ...
  6. Those set on the current target (not in a config).
```
### <a name="var_cppflags_c"></a>**cppflags***: Flags passed to the pre-processor.
...
```
  ...
  6. Those set on the current target (not in a config).
```
### <a name="var_cppflags_cc"></a>**cppflags***: Flags passed to the pre-processor.
...
```
  ...
  6. Those set on the current target (not in a config).
```
### <a name="var_cppflags_objc"></a>**cppflags***: Flags passed to the pre-processor.
...
```
  ...
  6. Those set on the current target (not in a config).
```
### <a name="var_cppflags_objcc"></a>**cppflags***: Flags passed to the pre-processor.
...
```
### <a name="var_description"></a>**description**: Description of the action.
...
```
### <a name="var_interpreter"></a>**interpreter**: Interpreter to use for action scripts.

```
  Path to the interpreter to run an action script with. This must be either an
  absolute system path (not a source absolute path) or an executable found in
  the environment search path. If unspecified, the default Python interpreter
  is used (see "gn help python_path", "gn help action" and
  "gn help action_foreach").
```
...
```
### <a name="var_sys_include_dirs"></a>**sys_include_dirs**: Additional system include directories.

```
  A list of source directories.

  The directories in this list will be added to the system include path for the
  files in the affected target.

  System include directories are searched after all normal include directories
...
```
  ...
  {{source_extension}}
      The extension of the source file. It includes the leading dot. This will
      generally be used for specifying a transformation from a source file to a
      destination file with the same extension but different name. Empty string
      means no extension.
        "//foo/bar/baz.txt" => ".txt"
  ...
  {{source_out_dir}}
      ...
      relative to the build directory. This must be different than the target's
